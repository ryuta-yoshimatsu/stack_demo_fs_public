...
The databricks CLI bundles top level is defined by file `stack_demo_fs_public/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`stack_demo_fs_public/assets/model-workflow-asset.yml`)
 - batch inference workflow (`stack_demo_fs_public/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`stack_demo_fs_public/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`stack_demo_fs_public/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`stack_demo_fs_public/assets/ml-artifacts-asset.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `stack_demo_fs_public/databricks.yml` and `stack_demo_fs_public/assets/ml-artifacts-asset.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-stack_demo_fs_public-model     | /dev-stack_demo_fs_public-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-stack_demo_fs_public-model | /staging-stack_demo_fs_public-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-stack_demo_fs_public-model    | /prod-stack_demo_fs_public-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-stack_demo_fs_public-model    | /test-stack_demo_fs_public-experiment    |
...
2. Complete the `TODO` in `stack_demo_fs_public/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `stack_demo_fs_public/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `stack_demo_fs_public/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-stack_demo_fs_public-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-stack_demo_fs_public-batch-inference-job`
that runs the notebook under `stack_demo_fs_public/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-stack_demo_fs_public-batch-inference-job`.
...
`test-stack_demo_fs_public-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-stack_demo_fs_public-batch-inference-job`.
